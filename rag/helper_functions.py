...
def retrieve_context_per_question(question, chunks_query_retriever):
    """
    Retrieves relevant context for a given question using the chunks vector store retriever.

    Args:
        question: The question to retrieve context for.
        chunks_query_retriever: Either a VectorStoreRetriever or the original VectorStore.

    Returns:
        A list of document contents (strings).
    """

    # Version-proof way to get documents
    try:
        # This works in newer versions
        docs = chunks_query_retriever.get_relevant_documents(question)
    except AttributeError:
        # Fallback for versions where VectorStoreRetriever lacks get_relevant_documents
        # Access the underlying vectorstore
        docs = chunks_query_retriever.vectorstore.similarity_search(question, k=2)
...
def get_langchain_embedding_provider(provider: EmbeddingProvider, model_id: str = None):
    ...
    """
    if provider == EmbeddingProvider.ALL_MINI:
        return HuggingFaceEmbeddings(model_name=MINIML_MODEL_PATH)
